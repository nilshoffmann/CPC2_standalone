...
def __main():
	...
	Common_group.add_option("-o",dest="outfile",help="output file [Default: cpc2output.txt]",metavar="FILE",type="string",default="cpc2output")
	Common_group.add_option("-r",dest="reverse",help="also check the reverse strand [Default: FALSE]",action="store_true")
	Common_group.add_option("--ORF",dest="ORF",help="output the putative peptide [Default: FALSE]",action="store_true")
...
def calculate_potential(fasta,strand,output_orf,outfile):
	...
	for seq in seqio.fasta_read(fasta):
		...
		if output_orf == 1:
			output_line = [seqid,len(seqRNA),pep_len,fickett_score,isoelectric_point,orf_fullness,newseqprot]
		else:
